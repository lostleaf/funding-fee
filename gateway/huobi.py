...
class HoubiGateway(BaseGateway):
    ...
    def get_swap_funding_fee_rate_history(self, symbol):
        ...
        for i in range(1, 3): 
            url = f"{prefix}/swap_historical_funding_rate" + \
              "?contract_code={symbol}&page_size=50&page_index={i}"
            resp = retry_getter(lambda: requests.get(url), raise_err=True)
            resp_data = resp.json()
